...
void sone::utils::defaultConnectionCallback(const TcpConnectionPtr &conn)
{
...
void sone::utils::defaultMessageCallback(const TcpConnectionPtr &,
                                        Buffer *buf,
                                        Timestamp)
{
    buf->retrieveAll();
...
void TcpConnection::send(const void *data, int len)
{
    if (state_ == kConnected)
    {
        ...
        else
        {
            ...
            loop_->runInLoop(std::bind(fp, this, data, len));
        }
    }
}
...
void TcpConnection::send(Buffer *buf)
{
    if (state_ == kConnected)
    {
        ...
        {
            void (TcpConnection::*fp)(const void* data, size_t len) = &TcpConnection::sendInLoop;
            loop_->runInLoop(std::bind(fp, this, buf->peek(), buf->readableBytes()));
        }
    }
}

...
namespace sone
{
    namespace detail
    {
        ...
        utils::Timestamp g_startTime = utils::Timestamp::now();
...
utils::Timestamp ProcessInfo::startTime()
{
    return g_startTime;
}
...
{
    const std::string& stat = procStatus();
    size_t lp = stat.find('(');
...
{
    int result = 0;
    const std::string& status = procStatus();
